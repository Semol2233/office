...
class daulycost_list(LoginRequiredMixin,ListView):
    ...
    queryset = dailybilling.objects.filter(date__range=datedata).exclude((Q(cost_profile__cost_name__contains='Fiber') | Q(cost_profile__cost_name__contains='Othoers') | Q(cost_profile__cost_name__contains='Onu')))
...
class area_bill(LoginRequiredMixin,ListView):
    ...
    def get_queryset(self):
        return monthlybill.objects.filter(Bill_collection_type__collection__startswith=self.kwargs['Bill_collection_type__collection'],bill_col_area__area_col__startswith=self.kwargs['bill_col_area__area_col'],month__month__startswith=month_col).exclude(activities__act_line__startswith="declined")
