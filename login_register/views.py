...
class Fiber(LoginRequiredMixin,ListView):
    ...
    queryset = dailybilling.objects.filter(cost_profile__cost_name__contains='Fiber',date__range=datedata)


class Onu(LoginRequiredMixin,ListView):
    context_object_name = 'fulllist'
    model = dailybilling
    template_name= 'goninda/cost.html'
    queryset = dailybilling.objects.filter(cost_profile__cost_name__contains='Onu',date__range=datedata)



class Otherss(LoginRequiredMixin,ListView):
    context_object_name = 'fulllist'
    model = dailybilling
    template_name= 'goninda/cost.html'
    queryset = dailybilling.objects.filter(cost_profile__cost_name__contains='Others',date__range=datedata)
...
class montlybillview(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
         ...
         alldata = monthlybill.objects.filter(month__month__startswith=month).order_by('user_id').exclude(activities__act_line__startswith="declined")
...
class routersell(LoginRequiredMixin,ListView):
    ...
    queryset = router.objects.exclude(date_router__range=routersdfcd,payment_methogd__rpay__contains='Cash')
...
class duebill(ListView):
    ...
    def get_context_data(self, **kwargs):
         ...
         context['unpaidwuser'] = monthlybill.objects.filter(payment_status=False,month__month__startswith=month).exclude(activities__act_line__startswith="declined",Bill_collection_type__collection__startswith="Collect_home",Bill_collection_type__collection__startswith="Collcet_shop")
         context['unpaiduser'] = monthlybill.objects.filter(payment_status=False,month__month__startswith=month,).exclude(activities__act_line__startswith="declined",Bill_collection_type__collection__startswith="Collect_home",Bill_collection_type__collection__startswith="Collcet_shop").count()
...
class bill_collection(LoginRequiredMixin,ListView):
    ...
    def get_context_data(self, **kwargs):
         context = super(bill_collection, self).get_context_data(**kwargs)
