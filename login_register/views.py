...
class SFSF(ListView):
    ...
    def get_context_data(self, **kwargs):
         ...
         context['unpaiduser'] = monthlybill.objects.filter(payment_status=False,month__month__startswith=month,).exclude(activities__act_line__startswith="declined",Bill_collection_type__collection__startswith="DF").count()
         context['nextuser'] = monthlybill.objects.filter(payment_status=True,pay_date__range=months)
